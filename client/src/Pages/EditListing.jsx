...
const CreateListingForm = () => {
  ...
  const [imagePath, setImagePath] = useState(currentListing.images);
  ...
  const handleFormSubmit = async (e) => {
    ...
    try {
      const res = await fetch(`/api/listing/${currentListing._id}`, {
        method: "PUT",
        headers: {
      ...
      if (!res.ok) {
        const errorText = await data.error;
        setError(errorText);
      }
  ...
  return (
    <form className="flex flex-col items-center  mt-9 ">
      <h2 className="text-2xl font-semibold mb-4">Edit a Listing</h2>
